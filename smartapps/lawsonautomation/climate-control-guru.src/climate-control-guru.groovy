 ...
 */
definition(
    name: "Climate Control Guru",
    namespace: "LawsonAutomation",
    author: "Tom Lawson",
    description: "This app implements night and day venting using whole house fans and/or other ventilation equipment by strategically introducing outside air. " +
...
def mainPage() {
    ...
    dynamicPage(name: "mainPage", install: true, uninstall: true) {  
        if (state.skew != null) {
    		section("Climate Control Guru Mode") {
        		if (state.skew > 0) {
        			paragraph "HEATING"
            	} else {
        			paragraph "COOLING"
		...
		section("Version 1.2.3 - Copyright © 2016 Thomas Lawson. " +
...
def UsersGuide() {
    dynamicPage(name: "UsersGuide", title: "User's Guide", nextPage: "mainPage") {
        ...
        section("Tips and Tricks") {
        	paragraph "Look in 'Notifications' of the SmartThings app to see what Climate Control Guru is doing."
...
def initInsideTempAndHumidity() {
    ...
    if (currentState) {
    	...
    	sendNotificationEvent("Climate Control Guru Error:  Unable to obtain initial inside temperature!")
    }
	...
	if (myThermostats) {
    	if (state.wholeHouseFans == state.On || openWindowCnt() != 0) {
        	...
        	if (state.thermostats != state.Heating) {
    			...
    			myThermostats.heat()
    			sendNotificationEvent("Climate Control Guru:  Thermostat(s) set to Heat mode")
            }
    	} else {
        	if (state.thermostats != state.Cooling) {
        		state.thermostats = state.Cooling
        		myThermostats.cool()
    			sendNotificationEvent("Climate Control Guru:  Thermostat(s) set to Cooling mode")
            }
    	}
    }
}
...
def turnOn(device, String devName, String extName) {
	...
	// don't send an open command if closing as it's already open
    	if (state[devName] == state.Closing) {
    		...
    		sendNotificationEvent("Climate Control Guru: ${action} ${extName}")
    	}
    }
}
...
def openUp(device, String devName, String extName) {
    ...
    if (state[devName] == state.Closing) {
    	...
    	state[devName] = state.Open 
    	device.open()
    	sendNotificationEvent("Climate Control Guru: Opened ${extName}")
    }
}
...
def closeUp(device, String devName, String extName) {
	...
	if (device && state[devName] != state.Closed) {
    	state[devName] = state.Closed    
   		device.close()
    	sendNotificationEvent("Climate Control Guru: Closed ${extName}")
    }
}
    ...
    if (cond) {
        ...
        if (myHumiditySensor) {
        	...
        	if (state.outsideHumidity == null) {
            	sendNotificationEvent("Climate Control Guru Error:  Unable to obtain outside humidity!")
                state.outsideHumidity = 20
        	}
...
def getHourlyConditions() {
    ...
    if (!resp) {
        ...
        LOG("Climate Control Guru Error: Could not obtain forecast information!")
    ...
    if (outsideHourly) {
        ...
        LOG("Climate Control Guru Error:  Could not obtain the hourly forecast values!")
    	state.forecastFailCntr = state.forecastFailCntr + 1
        return 0
    }
...
def makeComparisons() {	
    ...
    if (state.currentConditionsFailCntr > 6 || state.forecastFailCntr > 4) {
    	sendNotificationEvent("Climate Control Guru Error:  Check internet connection.  Can not access Weather Underground!")
		turnOffWHFsAndWindows()
		turnOffFans()
    } else if (smokeDetected()) { 
    	// turn off all fans and thermostats if there's smoke
	    sendNotificationEvent("Climate Control Guru Warning:  Smoke Detected!")
		turnOffFans()
    	turnOff(myThermostats, "thermostats", "Thermostats")
	} else {
        ...
        if (inWrongMode() || mySuspendOperation) {
			turnOffWHFsAndWindows()
           	...
           	if (state.windowsSwitch == state.On || state.wholeHouseFans == state.On) {
	    		sendNotificationEvent("Climate Control Guru:  Windows closed/Whole House Fans turned off due to rain")
            }
			turnOffWHFsAndWindows()
